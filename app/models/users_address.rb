class UsersAddress < ApplicationRecord
  ...
  validates :address_first_name_kana, :address_last_name_kana, format: { with: /\A[ぁ-んー－]+\z/, message: "は全角ひらがなで入力してください" }

  validates :post_number, format: { with: /\A\d{7}\z/, message: "はハイフン無しの7桁の番号を入力してください" }
end
